                                ...
                                completionBlock:(GetAuthCookieDataCompletionBlock)block
{
    if (_httpRequestOpManager) {
        ...
        NSURLSessionDataTask *operation = [_httpRequestOpManager GET:server
                                                             parameters:nil success:^(NSURLSessionTask *operation, id responseObject) {
                                                                 NSDictionary *responseDict = nil;
                                                                 
                                                                 if ([responseObject isKindOfClass:[NSDictionary class]]) {
                                                                     responseDict = responseObject;
                                                                 } else {
                                                                     responseDict = @{@"error" : responseObject};
                                                                 }
                          ...
                          completionBlock:(GetAccessTokenCompletionBlock)block
{
    if (_httpRequestOpManager) {
        ...
        [_httpRequestOpManager setTaskWillPerformHTTPRedirectionBlock:^NSURLRequest * _Nonnull(NSURLSession * _Nonnull session, NSURLSessionTask * _Nonnull task, NSURLResponse * _Nonnull response, NSURLRequest * _Nonnull request) {
            if (response == nil) {
                return request;
            } else {
