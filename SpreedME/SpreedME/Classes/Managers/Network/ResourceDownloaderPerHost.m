...
- (void)startTaskPrioriOS7:(ResourceDownloadTask *)task
{
	if (_httpRequestOpManager) {
		...
		NSURLSessionTask *operation = [_httpRequestOpManager GET:urlString parameters:nil success:^(NSURLSessionTask *operation, id responseObject) {
			[self hasFinishedTask:task withResponseObject:responseObject error:nil];
			...
			if ([error isKindOfClass:[NSData class]]) {
				/*NSUInteger dataLength = [operation.response length];
				if (dataLength) {
					[[NSNotificationCenter defaultCenter]
					 postNotificationName:STNetworkDataBytesHaveBeenReceivedNotification
					 object:nil
		...
		uint64_t requestSize = STCalculateNSURLRequestSize(operation.originalRequest);
...
{
	for (NSURLSessionTask *operation in _httpRequestOpManager.operationQueue.operations) {
