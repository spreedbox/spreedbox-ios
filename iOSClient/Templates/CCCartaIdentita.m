...
- (id)initWithDelegate:(id <CCCartaIdentitaDelegate>)delegate fileName:(NSString *)fileName uuid:(NSString *)uuid fileID:(NSString *)fileID isLocal:(BOOL)isLocal serverUrl:(NSString *)serverUrl
{
    ...
    if (self) {
        ...
        section = [XLFormSectionDescriptor formSectionWithTitle:NSLocalizedString(@"_id_card_", nil)];
        [form addFormSection:section];
...
- (void)viewDidLoad
{
    ...
    [templates setImageTitle:NSLocalizedString(@"_id_card_", nil) conNavigationItem:self.navigationItem reachability:[app.reachability isReachable]];
...
- (IBAction)savePressed:(UIBarButtonItem * __unused)button
{
    ...
    fileNameModel = [templates salvaForm:form fileName:self.fileName uuid:self.uuid modello:@"cartaidentita" icona:@"cartaidentita"];
